...
const MOBILE_COPY_MOTION_STOPS = [[0, 0], [0.24, 0], [0.35, 16], [0.46, 0], [1, 0]];
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const mobileCopyOffset =
        mobileMode && !reduceMotion
          ? interp(progress, MOBILE_COPY_MOTION_STOPS)
          : 0;
      ...
      if (mobileMode) {
        sticky.style.setProperty(
          "--mobile-hero-copy-opacity",
          progress < MOBILE_COPY_SWAP_AT ? "1" : "0",
        );
        sticky.style.setProperty(
          "--mobile-bridge-copy-opacity",
          progress < MOBILE_COPY_SWAP_AT ? "0" : "1",
        );
        sticky.style.setProperty(
          "--mobile-copy-offset",
          `${mobileCopyOffset.toFixed(2)}px`,
        );
      }
